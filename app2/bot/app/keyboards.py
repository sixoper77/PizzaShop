...
from .data import get_categories, get_data
    ...
    for category in all_categories:
        keyboard.row(
            InlineKeyboardButton(
                text=f"{category['name']}", callback_data=f"category_{category['slug']}"
            )
        )
...
async def get_items_by_category_slug(category_slug, state: FSMContext):
    ...
    for index, item in enumerate(products_by_categories):
        keyboard.row(
            InlineKeyboardButton(
                text=item["name"], callback_data=f"product_{item['id']}"
            )
        )
...
def switch_item(product_index, product_id, products):
    ...
    if product_index > 0:
        ...
        row.append(
            InlineKeyboardButton(
                text="Прошлая пицца", callback_data=f"product_{prev_product_id}"
            )
        )
    if product_index < len(products) - 1:
        ...
        row.append(
            InlineKeyboardButton(
                text="Следующая пицца", callback_data=f"product_{next_product_id}"
            )
        )
    ...
    keyboard_buttons.append(
        [InlineKeyboardButton(text="Cоздать ордер", callback_data="create_order")]
    )
    keyboard_buttons.append(
