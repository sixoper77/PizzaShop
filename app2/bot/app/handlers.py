...
async def catalog(callback: CallbackQuery):
    ...
    if callback.message.text:
        await callback.message.edit_text(
            "Select product category", reply_markup=await kb.categories()
        )
    else:
...
async def all_products(callback: CallbackQuery, state: FSMContext):
    ...
    if callback.message.text:
        await callback.message.edit_text(
            "Список всех продуктов", reply_markup=await kb.products()
        )
    else:
        await callback.message.answer(
            "Список всех продуктов", reply_markup=await kb.products()
...
async def add_cart(callback: CallbackQuery, state: FSMContext):
    ...
    succes = await add_to_cart(
        {"product_id": product_id, "quantity": 1, "telegram_id": user_id}, user_id
    )
    print(succes)
...
async def process_postal_code(message: Message, state: FSMContext):
    ...
    items_text = "\n".join(
        [f"🍕 {item['name']} - {item['quantity']} шт." for item in cart_data["cart"]]
    )
    confirmation_text = (
...
async def confirm_order(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if response.get("status") == "success":
            kb.menu.inline_keyboard.append(
                [
                    InlineKeyboardButton(
                        text="Оплатить",
                        callback_data=f"pay_order_{response['order_id']}",
                    )
                ]
            )
            await callback.message.answer(
